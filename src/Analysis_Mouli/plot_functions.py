...
def plot_count_data(data_figure, periods, account, sessions, output_location):
    ...
    if account == "Total":
        ...
        if sessions == "main":
            ...
            if periods == "AP":
                data = data_figure.groupby("treatment")["total_pgg_investment"].value_counts(normalize=True).reset_index()
            else:
                data = data_figure[data_figure.period == 1].groupby("treatment")["total_pgg_investment"].value_counts(normalize=True).reset_index()
            sns.barplot(data, x="total_pgg_investment", y="proportion", hue="treatment", edgecolor="0.5", palette=["#191919","#666","#999"])
        elif sessions == "bots":
            if periods == "AP":
                data = data_figure["total_pgg_investment"].value_counts(normalize=True).reset_index()
            else:
                data = data_figure[data_figure.period == 1]["total_pgg_investment"].value_counts(normalize=True).reset_index()
            sns.barplot(data, x="total_pgg_investment", y="proportion", color = "#ccc", edgecolor="0.5")
        elif sessions == "both":
            hue_order = ["Single Group", "Multi-Split", "Multi-Shared (Humans)", "Multi-Shared (Bots Balanced)", "Multi-Shared (Bots Unbalanced)"]
            if periods == "AP":
                data = data_figure.groupby("treatment")["total_pgg_investment"].value_counts(normalize=True).reset_index()
            else:
                data = data_figure[data_figure.period == 1].groupby("treatment")["total_pgg_investment"].value_counts(normalize=True).reset_index()
            sns.barplot(data, x="total_pgg_investment", y="proportion", hue="treatment", edgecolor="0.5", hue_order=hue_order, palette=["#191919","#666","#999","#ccc"])
        # print(data)
        ...
        if sessions == "main":
            if periods == "AP":
                data = data_figure.groupby("treatment")["blue_investment"].value_counts(normalize=True).reset_index()
            else:
                data = data_figure[data_figure.period == 1].groupby("treatment")["blue_investment"].value_counts(normalize=True).reset_index()
            sns.barplot(data, x="blue_investment", y="proportion", hue="treatment", palette=["#000165", "#0103fd"], edgecolor="0.5")
        elif sessions == "bots":
            if periods == "AP":
                data = data_figure["blue_investment"].value_counts(normalize=True).reset_index()
            else:
                data = data_figure[data_figure.period == 1]["blue_investment"].value_counts(normalize=True).reset_index()
            sns.barplot(data, x="blue_investment", y="proportion", color = "#96b3d1", edgecolor="0.5")
        elif sessions == "both":
            hue_order = ["Multi-Split", "Multi-Shared (Humans)", "Multi-Shared (Bots Balanced)", "Multi-Shared (Bots Unbalanced)"]
            if periods == "AP":
                data = data_figure.groupby("treatment")["blue_investment"].value_counts(normalize=True).reset_index()
            else:
                data = data_figure[data_figure.period == 1].groupby("treatment")["blue_investment"].value_counts(normalize=True).reset_index()
            sns.barplot(data, x="blue_investment", y="proportion", hue="treatment", hue_order=hue_order, palette=["#000165", "#0103fd", "#96b3d1"], edgecolor="0.5")
        ...
        if sessions == "main":
            if periods == "AP":
                data = data_figure.groupby("treatment")["green_investment"].value_counts(normalize=True).reset_index()
            else:
                data = data_figure[data_figure.period == 1].groupby("treatment")["green_investment"].value_counts(normalize=True).reset_index()
            sns.barplot(data, x="green_investment", y="proportion", hue="treatment", palette=["#01310f", "#03952f"], edgecolor="0.5")
        elif sessions == "bots":
            if periods == "AP":
                data = data_figure["green_investment"].value_counts(normalize=True).reset_index()
            else:
                data = data_figure[data_figure.period == 1]["green_investment"].value_counts(normalize=True).reset_index()
            sns.barplot(data, x="green_investment", y="proportion", color="#06f84e", edgecolor="0.5")
        elif sessions == "both":
            hue_order = ["Multi-Split", "Multi-Shared (Humans)", "Multi-Shared (Bots Balanced)", "Multi-Shared (Bots Unbalanced)"]
            if periods == "AP":
                data = data_figure.groupby("treatment")["green_investment"].value_counts(normalize=True).reset_index()
                ...
                data = data_figure[data_figure.period == 1].groupby("treatment")["green_investment"].value_counts(normalize=True).reset_index()
            sns.barplot(data, x="green_investment", y="proportion", hue="treatment", hue_order=hue_order, palette=["#01310f", "#03952f", "#06f84e"], edgecolor="0.5")
        # print(data)
    ...
    # stat01, pval01 = stats.kruskal(data[data.treatment == "Single Group"]["total_pgg_investment"], data[data.treatment == "Multi-Split"]["total_pgg_investment"])
    # print("Single vs. Split: ", round(pval01,3))
    # stat02, pval02 = stats.kruskal(data[data.treatment == "Single Group"]["total_pgg_investment"], data[data.treatment == "Multi-Shared"]["total_pgg_investment"])
    # print("Single vs. Shared: ", round(pval02,3))
    # stat12, pval12 = stats.kruskal(data[data.treatment == "Multi-Split"]["total_pgg_investment"], data[data.treatment == "Multi-Shared"]["total_pgg_investment"])
...
def plot_avg_amount_account_byPeriod(data_figure, account, sessions, output_location):
    ...
    if account == "Total":
        hue_order = ["Single Group", "Multi-Split", "Multi-Shared (Humans)", "Multi-Shared (Bots Balanced)", "Multi-Shared (Bots Unbalanced)"]
        data = data_figure.groupby(["treatment", "period"])["total_pgg_investment"].mean().reset_index()
        sns.lineplot(data, x="period", y="total_pgg_investment", hue="treatment", hue_order=hue_order, palette=["#191919","#666","#999","#ccc"], style="treatment")
        plt.ylabel("Average Total\nPublic Good Investment", fontsize = 16)
        plt.legend()
    elif account == "Blue":
        hue_order = ["Multi-Split", "Multi-Shared (Humans)", "Multi-Shared (Bots Balanced)", "Multi-Shared (Bots Unbalanced)"]
        data = data_figure.groupby(["treatment", "period"])["blue_investment"].mean().reset_index()
        sns.lineplot(data[data.treatment != "Single Group"], x="period", y="blue_investment", hue="treatment", hue_order=hue_order, palette=["#000165", "#0103fd", "#96b3d1"], style="treatment")
        plt.ylabel("Average Blue\nPublic Good Investment", fontsize = 16)
        plt.legend()
    elif account == "Green":
        hue_order = ["Multi-Split", "Multi-Shared (Humans)", "Multi-Shared (Bots Balanced)", "Multi-Shared (Bots Unbalanced)"]
        data = data_figure.groupby(["treatment", "period"])["green_investment"].mean().reset_index()
        sns.lineplot(data[data.treatment != "Single Group"], x="period", y="green_investment", hue="treatment", hue_order=hue_order, palette=["#01310f", "#03952f", "#06f84e"], style="treatment")
        plt.ylabel("Average Green\nPublic Good Investment", fontsize = 16)
        plt.legend()
    elif account == "Personal":
        hue_order = ["Single Group", "Multi-Split", "Multi-Shared (Humans)", "Multi-Shared (Bots Balanced)", "Multi-Shared (Bots Unbalanced)"]
        data = data_figure.groupby(["treatment", "period"])["personal_account"].mean().reset_index()
        sns.lineplot(data, x="period", y="personal_account", hue="treatment", hue_order=hue_order, palette=["#191919","#666","#999","#ccc"], style="treatment")
...
def plot_diff_BlueGreen(data_figure, output_location):
    ...
    data_figure["diff_player_group_investment"] = data_figure["blue_investment"] - data_figure["green_investment"]
    data_figure["L_diff_others_group_investment"] = (data_figure["L_others_blue_investment"] - data_figure["L_others_green_investment"])/3
    ...
    # print(data_figure[(data_figure.treatment  != "Single Group") & (data_figure.period ==3)][["treatment", "L_diff_others_group_investment", "diff_player_group_investment"]].head(10))
    
    for treatment in ["Multi-Split", "Multi-Shared (Humans)", "Multi-Shared (Bots Balanced)", "Multi-Shared (Bots Unbalanced)"]:
...
def plot_avg_inv_diffaccount_byPeriod(data_figure, column_list, color_list, style_list, label_list, accounts, output_location):
    ...
    for treatment in ["Multi-Split", "Multi-Shared (Humans)", "Multi-Shared (Bots Balanced)", "Multi-Shared (Bots Unbalanced)"]:
        ...
        for i, column in enumerate(column_list):
            ...
            sns.lineplot(data = data, x = "period", y= data[column], color=color_list[i], linestyle = style_list[i], label=label_list[i], errorbar=None)
            # print(label_list[i])
...
def plot_order_effect(data_figure, output_location):
    ...
    extension_avg_blue_true = df_true.groupby('period')['blue_investment'].mean()
    extension_avg_green_true = df_true.groupby('period')['green_investment'].mean()

    extension_avg_blue_false = df_false.groupby('period')['blue_investment'].mean()
    extension_avg_green_false = df_false.groupby('period')['green_investment'].mean()

    extension_avg_blue_true_df = extension_avg_blue_true.to_frame().reset_index()
    extension_avg_blue_true_df.rename(columns={"blue_investment":0}, inplace=True)
    extension_avg_blue_false_df = extension_avg_blue_false.to_frame().reset_index()
    extension_avg_blue_false_df.rename(columns={"blue_investment":0}, inplace=True)
    extension_avg_green_true_df = extension_avg_green_true.to_frame().reset_index()
    extension_avg_green_true_df.rename(columns={"green_investment":0}, inplace=True)
    extension_avg_green_false_df = extension_avg_green_false.to_frame().reset_index()
    extension_avg_green_false_df.rename(columns={"green_investment":0}, inplace=True)
    ...
    combined_avg_df.rename(columns={"period":"Period", 0:"Average Account Investment"}, inplace=True)
